...
class TenantDatabaseMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        if not tenant_code:
            path = request.path
            if path.startswith(f'/{config("ADMIN_URL")}/default/') or path.startswith(f'/{config("MANAGER_URL")}/default/'):
                tenant_code = 'default'
            elif path.startswith(f'/{config("ADMIN_URL")}/test/') or path.startswith(f'/{config("MANAGER_URL")}/test/'):
                tenant_code = 'test'
            ...
            elif path.startswith(f'/{config("ADMIN_URL")}/fashion-jewelry/') or path.startswith(f'/{config("MANAGER_URL")}/fashion-jewelry/'):
                tenant_code = 'fashion-jewelry'
        ...
        tenant_mapping = {
            ...
            'fashion-jewelry': 'fashion_jewelry_db',
        }
